...
VI the_most_popular_divisors(VI &t, int n, int &max_count){
	...
	REP(i,0,n-1){ // O(n)
		++histogram[ t[i] ];
	} // now: histogram[x] = number of occurences of x in our multiset
	
	REP(i,3,MAX_T){ // O(MAX_T)
		histogram[i] += histogram[i-1];
	} // now: histogram[x] = amount of occurences of numbers <= x in our multiset
	...
	REP(i,2,MAX_T){ // for each possible divisor
		...
		if( n - histogram[i-1] >= max_count ){ // if |{x: x >= i and x in t}| >= max_count (if a given divisor has a chance to be the most popular)
			
			FOR(j,i,MAX_T,i){ // for each multiple of divisor i
				tmp_count += histogram[j] - histogram[j-1]; // we consider the multiple "j" of number "i"
			}
...
In:
10
2 4 6 8 10 100 1000 10000 100000 1000000
Out:
2
10
